...
Last updated: 2026-06-09T05:12:00Z
Last run: #223 — Lens: LLM integration quality
...
### Run #222 — 2026-06-09 — Lens: Deployment / Docker
**Scope:** Fifteenth Deployment/Docker pass. Full reads of: `docker-compose.yml`, `geointellisense-analytics/Dockerfile`, `geointellisense-ingestion/Dockerfile`, `Caddyfile`, `geointellisense-analytics/.dockerignore`, `geointellisense-ingestion/.dockerignore`, `geointellisense-analytics/requirements.txt`, `geointellisense-ingestion/Cargo.toml`, `geointellisense-analytics/app/main.py`, `geointellisense-analytics/app/claude.py`, `db/init/01-extensions.sql`, `db/init/02-migrations.sh`. Cross-checked against Active Recommendations and archived Deployment/Docker runs #12, 27, 42, 57, 72, 87, 102, 117, 132, 147, 162, 177, 192, 207 to confirm findings are new.

**Findings:**

- OBSERVATION: `claude.py:231,233` — The `execute_tool()` function's `get_air_quality` branch makes two HTTP calls in Docker that both silently fail. The primary call at line 231 (`await client.get(f"http://localhost:3002/api/aqi-snapshot", params=params)`) hits the analytics FastAPI service's own listener on the analytics container — but `/api/aqi-snapshot` is NOT registered in any of the 30+ routers imported in `main.py`; it is exclusively served by the Rust ingestion service via Caddy's `handle /api/aqi-snapshot { reverse_proxy ingestion:3001 }` (Caddyfile line 7). FastAPI returns 404. The `if resp.status_code != 200` branch at line 232 triggers the fallback at line 233: `await client.get("http://localhost:3001/api/aqi-snapshot")` — but inside the `analytics` Docker container, `localhost:3001` is the analytics container's own loopback at port 3001, where nothing is listening. This raises `httpx.ConnectError: [Errno 111] Connection refused`, caught at `claude.py:271` and returned as `{"error": "Tool execution failed: ..."}` to Claude. Result: Claude's `get_air_quality` tool call ALWAYS returns an error object in the Docker deployment; the model never receives live AQI data through tool use. PROPOSAL: Replace `claude.py:233` with `await client.get("http://ingestion:3001/api/aqi-snapshot", params=params)` to use the Docker Compose service name — L/L effort (1 line; restores AQI data to Claude's tool-use path in Docker).

- OBSERVATION: `docker-compose.yml:30` — Redis is configured with both `--appendonly yes` (AOF persistence) and `--maxmemory-policy allkeys-lru` (evicts any key regardless of TTL under memory pressure). The analytics service uses Redis exclusively as a TTL-based ephemeral cache for poll loop results (AQI readings, fire detections, water levels, inversion events — confirmed by `claude.py:29` which stores sessions in-process in `_sessions: dict`, not in Redis). With `--appendonly yes`, every `SET` and `SETEX` call is durably written to `/data/appendonly.aof`. When `allkeys-lru` evicts a cached entry after 256 MB is reached, those eviction DELs are also appended to the AOF. Since all cached values are repopulated by background poll loops within seconds of a Redis restart, AOF persistence provides zero recovery value. The AOF file grows continuously until an auto-rewrite triggers (default: 64 MB base + 100% growth ≥ 128 MB accumulated). Under sustained write load from multiple poll loops, the `redisdata` volume can grow to several GB before rewrite, and the rewrite itself is a CPU/I/O spike that competes with analytics tasks. PROPOSAL: Change `docker-compose.yml:30` from `redis-server --appendonly yes ...` to `redis-server --appendonly no --save "" --maxmemory 256mb --maxmemory-policy allkeys-lru` — L/L effort (1 line; eliminates continuous disk writes and unbounded volume growth for a pure-cache Redis instance whose data has no recovery value).

- OBSERVATION: `geointellisense-ingestion/Dockerfile:11` — The dummy Rust build step `RUN cargo build --release 2>/dev/null || true` is intended to pre-cache crate downloads for layer caching, where the expected failure is `error[E0601]: main function not found` (due to the empty `main.rs`). The `2>/dev/null` redirect discards ALL stderr output — including Cargo registry network errors (`error: failed to get crate 'tokio'... network: Unable to resolve host 'crates.io'`). The `|| true` forces exit code 0 regardless. When the Cargo registry is unreachable (corporate proxy, transient DNS failure, air-gapped CI), this step records a "success" in Docker's layer cache with an empty `~/.cargo/registry/`. The subsequent real build at `Dockerfile:14` (`RUN touch src/main.rs && cargo build --release`) then fails with cryptic linker errors or `error[E0425]: cannot find function` messages that have no visible connection to the root cause (registry unreachable), and the cached "empty registry" layer may persist across rebuilds if `Cargo.lock` hasn't changed. PROPOSAL: Replace `2>/dev/null` with nothing — keep stderr visible: `RUN cargo build --release || true` — L/L effort (1 character deletion; preserves Cargo network error output for debugging while still force-succeeding on the expected E0601 compile error).

- OBSERVATION: `docker-compose.yml:109-114` and `geointellisense-analytics/app/routes/health.py:6-12` — The analytics healthcheck `python -c "import urllib.request; urllib.request.urlopen('http://localhost:3002/api/health')"` passes no `timeout` argument to `urlopen()` (default: inherits the global socket timeout, which is None = blocking indefinitely). This interacts destructively with the ML retraining background task: `start_retrain_scheduler()` at `main.py:54` periodically calls the `aqi_model.py` `fit()` method, which runs `RandomForestRegressor.fit()` synchronously on the uvicorn event loop thread (not offloaded to `run_in_executor`). During training, the event loop is blocked; `GET /api/health` is queued behind the blocked loop and cannot respond within `timeout: 5s`. Docker marks the analytics container `unhealthy` after `retries: 5` (i.e., 5 consecutive missed probes × 10s interval = ~50s). While `depends_on` does not stop the gateway after startup, Docker's `restart: unless-stopped` restarts the container — interrupting training mid-fit, destroying the partially-trained in-memory model, and dropping all in-progress chat and analysis HTTP connections. The ML training is falsely interpreted as a crash. PROPOSAL: (a) Wrap `model.fit()` in `await asyncio.get_event_loop().run_in_executor(None, model.fit, X, y)` in `aqi_model.py` to avoid blocking the event loop; (b) add `timeout=3` to `urlopen()` in `docker-compose.yml:110` — L/M effort (~5 lines; eliminates false health failures and prevents training interruption via restart).

**Proposed actions:**
- Replace `claude.py:233` hardcoded `http://localhost:3001/api/aqi-snapshot` with `http://ingestion:3001/api/aqi-snapshot` — L/L effort (1 line; restores AQI tool-use data for Claude in Docker)
- Change `docker-compose.yml:30` Redis command to `--appendonly no --save ""` — L/L effort (1 line; stops unbounded AOF growth for pure-cache Redis)
- Remove `2>/dev/null` from `geointellisense-ingestion/Dockerfile:11` dummy build step — L/L effort (1 edit; surfaces registry network errors during build)
- Offload `model.fit()` to `run_in_executor` in `aqi_model.py` and add `timeout=3` to analytics healthcheck — L/M effort (~5 lines; prevents false-unhealthy during ML training)
